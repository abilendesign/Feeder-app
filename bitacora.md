...
| 2026-06-06 | Login REPUESTO (Supabase Auth) + middleware. GUARDADO de cards: columna `data jsonb` en property_cards (correr supabase/add-data-column.sql), endpoint /api/cards (insert/update con sesión del usuario, RLS), botón Guardar en la tarjeta. PÁGINA DE PERFIL /perfil: muestra correo, # de propiedades guardadas y grid de cards con su nombre; botón "Volver al chat" y "Cerrar sesión". Botón "Perfil" arriba-izquierda del mapa. Fotos aún NO van a Storage (se guardan como [] en data). |
| 2026-06-06 | FIX imagen: 2 causas. (1) Fotos grandes de celular superaban el límite de body de Vercel -> ahora se comprimen/reducen en el navegador (canvas, max 1600px, JPEG 0.7) antes de enviar. (2) gemini-2.5-pro tiene CUOTA 0 en free tier; las imágenes >1.5MB escalaban a pro y fallaban -> ahora modelHeavy también es gemini-2.5-flash. Imagen probada OK. |
| 2026-06-06 | Visión (Imagen/Escanear) PROBADA: Gemini leyó una imagen de anuncio y extrajo tipo, operación, recámaras, baños, parking y ubicación (con pin). Precio/m² fallaron solo por la baja calidad de la imagen de prueba (placeholder); con foto real nítida debería capturar todo. La generación de imágenes de Gemini sí está sin cuota en free tier, pero la visión (input) funciona bien. |
| 2026-06-06 | Audio PROBADO y funcionando: el backend (Gemini transcribe) confirmado OK con WAV (~2s en caliente; arranque en frío ~80s). Como el navegador graba webm (no garantizado en Gemini), ahora se convierte a WAV en el cliente (AudioContext + encoder WAV) antes de enviar. /api/transcribe con maxDuration=60. NOTA: OpenAI quedó SIN CUOTA (insufficient_quota), así que no es opción de respaldo ahora. |
| 2026-06-06 | Dos links con nombre en Vercel (mismo app responsivo): https://feeder-celular.vercel.app y https://feeder-pc.vercel.app. Creados con `vercel alias set` (la API REST con el token de la CLI da invalidToken/403). IMPORTANTE: son aliases a un deploy específico; en CADA deploy hay que re-sincronizarlos: `vercel alias set <nuevo-deploy> feeder-celular.vercel.app` y `feeder-pc.vercel.app`. El dominio auto-actualizable sigue siendo feeder-app-tan.vercel.app. |
| 2026-06-05 | Tarjeta rediseñada al modelo INMOBILIARIO y totalmente EDITABLE (8 secciones: header, título, precio con $/m² automático, detalles, descripción, ubicación, fotos, fuente). Schema/extracción de IA alineados a campos de propiedad (Gemini). Mapa usa lat/lng. Botón Salir (logout). Probado OK. PENDIENTE: guardar en Supabase (la tarjeta aún vive en memoria). |
| 2026-06-05 | Supabase conectado. Esquema SQL creado (property_cards, chat_messages, documents, transcriptions) + RLS por usuario + bucket Storage 'documents'. App de BIENES RAÍCES (Panamá). Login email/contraseña (Supabase Auth) + middleware de sesión: el sitio ahora exige login. Llaves en .env.local y Vercel. PENDIENTE: alinear el esquema de extracción de la IA y la tarjeta a los campos inmobiliarios, y wirear el guardado (card/chat/docs/transcripciones). |
| 2026-06-04 | MIGRACIÓN a GEMINI (para empezar gratis): refactor con Vercel AI SDK. `src/lib/ai.ts` centraliza el proveedor (cambiar a OpenAI = 3 líneas). /api/chat usa generateObject + gemini-2.5-flash (escala a gemini-2.5-pro). /api/transcribe usa generateText con audio (gemini-2.5-flash). Key de Google en .env.local y Vercel. Chat+visión probados OK en local y producción. AUDIO webm pendiente de probar en la app real (Gemini podría no aceptar webm). |
